...
/*************main tab信号槽*********************/
    ...
    connect(compareview,SIGNAL(deleteDiffSound(diffSound &)),&baseTree,SLOT(restoreTreeItemWidget(diffSound &)));
...
void QTrainMainWindow::windowInit()
{
    ...
    web_PPt->setFocusPolicy(Qt::NoFocus);
    web_PPt->hide();
...
{
    qDebug()<<__FUNCTION__<<data1.m_strSoundName<<data2.m_strSoundName;
    m_nState = 0;
...
void QTrainMainWindow::DiffSoundLoadInit(QByteArray &Data)
{
    ...
    if(m_DiffsoundInfo[0].isEmpty())
    {
        ...
        qDebug()<<__FUNCTION__<<"DATA1:=============="<<Data;
        return;
    }
    if( m_DiffsoundInfo[1].isEmpty())
    {
        qDebug()<<__FUNCTION__<<"Data2:"<<Data;
